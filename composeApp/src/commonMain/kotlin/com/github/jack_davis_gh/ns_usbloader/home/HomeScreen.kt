...
import nsusbloader.composeapp.generated.resources.Res
                    ...
                    Row(verticalAlignment = Alignment.CenterVertically,
                        ...
                        if (state.selectedFileNum == 0) {
                            val (protoIcon, protoStr) = when (state.activeProtocol) {
                                Protocol.Network -> Pair(Icons.Outlined.Wifi, "NET") //Res.string.tf_net)
                                Protocol.USB -> Pair(Icons.Outlined.Usb, "USB") //Res.string.tf_usb)
                            }
                            Icon(imageVector = protoIcon, contentDescription = null)
                            Text(protoStr) //stringResource(protoStr))
                        } else {
                ...
                actions = {
                    if (state.selectedFileNum == 0) {
                        ...
                        IconButton(callbacks::onUploadFileClicked) {
                            Icon(Icons.Outlined.UploadFile, //painterResource(Res.drawable.ic_upload_btn),
